# Backup of the original analytics.py before implementing multi-threaded agent-based evaluation.
        ...
        try:
            ...
            def run_single_test_case(idx, tc):
                ...
                initial_state = {
                    ...
                    "response_mode": "freeform",  # classify_intent_node will override
                ...
                # Unique thread_id per run to avoid LangGraph checkpoint collisions
                config = {"configurable": {"thread_id": f"eval_faith_{client_id}_{idx}_parallel"}}
                # Each thread gets its own agent instance to avoid shared-state issues
                ...
                return idx, query, contexts, answer, tool_acc
            ...
            with st.status(
                ...
                # Create per-row placeholders BEFORE threads start so they render immediately
                row_placeholders = []
                ...
                completed_count = [0]  # list so threads can mutate it
                lock = threading.Lock()
                ...
                with ThreadPoolExecutor(max_workers=max_workers) as executor:
                    ...
                    for future in as_completed(future_to_idx):
                        ...
                        try:
                            idx, query, contexts, answer, tool_acc = future.result()
                            results_map[idx] = (query, contexts, answer, tool_acc)
                            row_placeholders[idx].markdown(
                                f"✅ `[{idx+1}/{len(test_cases)}]` **{test_cases[idx]['id']}** — "
            ...
            # Each sample is scored in its own thread with fresh metric instances
            # to avoid any shared-state issues between concurrent Ragas calls.
            openai_api_key = os.environ.get("OPENAI_API_KEY")

            def score_single_sample(idx, question, contexts, answer):
                """Score one sample with Faithfulness + ResponseRelevancy."""
                # Fresh LLM / embeddings per thread — OpenAI client is stateless HTTP
